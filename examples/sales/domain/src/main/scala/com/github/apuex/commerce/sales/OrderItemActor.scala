...
object OrderItemActor {
  def props = Props[OrderItemActor]
  def name: String = "OrderItemActor"
}

class OrderItemActor () extends PersistentActor with ActorLogging {
  ...
  override def receiveRecover: Receive = {
    case evt: Event =>
      updateState(evt)
    case SnapshotOffer(_, x: OrderItemVo) =>
